...
- **リポジトリ登録が不要** — 開いた場所が git 配下なら自動で認識。メニューもバッジも
  git が効く場所にだけ現れ、それ以外では出ません
- **環境設定** — 常時監視したいリポジトリの指定、`git` バイナリと外部 diff ツールの指定、
...
拡張はホームディレクトリ全体を監視し、各アイテムがどのリポジトリに属するかを判定して、
Finder が表示中のリポジトリを `/Users/Shared/StatoiseGitShared` に公開します。本体アプリは
それらに対して `git status` を実行し（開いているリポジトリは毎周期、フォルダ内に並んでいる
だけのリポジトリはゆっくり巡回）、結果を同じディレクトリに書き戻して拡張が描画します。
